...
void pred_single_TwitterLDA(IntegerMatrix w, IntegerVector doc_users,
  ...
  update_counts_TwitterLDA(w, doc_users, alphastar, bV, TOPICS, D, N, zstar, yV,
                           WY1ZX_new, Zstar_new, Yv1_new, WY0_new, true);
    ...
    for (d = 0; d < D; d++) {
      ...
      for (n = 0; n < N(d); n++) {
        if(yV(d, n) == 1) {
          ...
          WY0_new(w(d,n)-1) = WY0_new(w(d,n)-1) + 1;
        }
      }
      // -----------------------------------------------------------------------
      ...
      Zstar_new(u, zstar(d)-1) = Zstar_new(u, zstar(d)-1) - 1;
      for (n = 0; n < N(d); n++) {
        WY1ZX_new(w(d,n)-1, zstar(d)-1) = WY1ZX_new(w(d,n)-1, zstar(d)-1) - yV(d, n);
      }
      ...
      for (n = 0; n < N(d); n++) {
        ...
        // full conditional probability
        double p_Yv0 = (bV(1) + N_sum-1 - Yv1 - Yv1_new) * (betaV(w(d,n)-1) + WY0(w(d,n)-1) + WY0_new(w(d,n)-1)) / (betaV_sum + sum(WY0) + sum(WY0_new));
        double p_Yv1 = (bV(0) + Yv1 + Yv1_new) * (betaV(w(d,n)-1) + WY1ZX(w(d,n)-1,zstar(d)-1) + WY1ZX_new(w(d,n)-1,zstar(d)-1)) / (betaV_sum + sum(WY1ZX(_,zstar(d)-1)) + sum(WY1ZX_new(_,zstar(d)-1)));
        // sample new value
